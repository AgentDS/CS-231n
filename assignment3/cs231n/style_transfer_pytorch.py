...
def gram_matrix(features, normalize=True):
    ...
    # use einsum for covariance calculation
    gram = torch.einsum('nchw,nkhw->nck', features, features)
    if normalize:
        gram = gram / (H * W * C)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    loss = 0.
    for layer, style_target, style_weight in zip(style_layers, style_targets, style_weights):
        gram_current = gram_matrix(feats[layer])
        loss += style_weight * torch.sum((gram_current - style_target) ** 2)
    return loss
...
def tv_loss(img, tv_weight):
    ...
    diff_h = img[:, :, 1:, :] - img[:, :, :-1, :]
    diff_w = img[:, :, :, 1:] - img[:, :, :, :-1]
    return tv_weight * (torch.sum(diff_h ** 2) + torch.sum(diff_w ** 2))
